...
class AssistenciaController extends Controller
{
    ...
    {
        $id_user = auth()->user()->id;
        $select = 'SELECT  organizers.name AS organizerName, events.name, events.date, events.hour, events.categories, events.link, events.photo, events.dayOfWeek, events.link, events.id
                        FROM `assistencias` 
