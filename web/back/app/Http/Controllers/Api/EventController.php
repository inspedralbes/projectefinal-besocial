...
class EventController extends Controller
{
    ...
    {
        $actualDate = getDate();
        $select = 'SELECT events.id AS id, organizers.name AS organizer, events.name AS name, events.date AS date, events.hour AS hour, organizers.address AS address, organizers.postal_code AS postal_code, organizers.city AS city, organizers.coords AS coords, events.link AS link, events.categories AS categories, events.photo AS photo, events.dayOfWeek AS dayOfWeek FROM events, organizers WHERE events.idOrganizer = organizers.id ';
        if ($request->search) {
